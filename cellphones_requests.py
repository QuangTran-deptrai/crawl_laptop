import argparse
import math
import time
from datetime import datetime, timezone, timedelta
import requests
import pandas as pd

def calculate_discount(current_price, original_price, scraped_discount=""):
    try:
        import re
        if 'liên hệ' in str(current_price).lower() or 'liên hệ' in str(original_price).lower():
            return ""
        c = int(re.sub(r'[^\d]', '', str(current_price)))
        o = int(re.sub(r'[^\d]', '', str(original_price)))
        if o > c and o > 0 and c > 0:
            percent = round((o - c) / o * 100)
            if percent <= 70:
                return f"-{percent}%"
    except Exception:
        pass
        
    if scraped_discount and str(scraped_discount).strip():
        d = str(scraped_discount).strip()
        if d.endswith('%') and not d.startswith('-'):
            d = f"-{d}"
        return d
        
    return ""

def fetch_page(page, keyword="laptop"):
    """Fetch 1 trang tu API GraphQL cua Cellphones"""
    url = 'https://api.cellphones.com.vn/v2/graphql/query'
    payload = {
        'query': '''
            query GetProductsByCateId{
                products(
                        filter: {
                            static: {
                                categories: ["380"],
                                province_id: 30,
                                stock: { from: 0 }
                            },
                            dynamic: {}
                        },
                        page: %d,
                        size: 50,
                        sort: [{view: desc}]
                    )
                {
                    general{
                        product_id
                        name
                        attributes
                        url_path
                    }
                    filterable{
                        price
                        special_price
                    }
                }
            }
        ''' % page
    }
    headers = {'Content-Type': 'application/json', 'User-Agent': 'Mozilla/5.0'}
    try:
        response = requests.post(url, json=payload, headers=headers, timeout=10)
        if response.status_code == 200:
            data = response.json()
            return data.get('data', {}).get('products', [])
    except Exception as e:
        print(f"    [X] Loi tai trang {page}: {e}")
    return []

def get_all_laptop_products():
    """Lay toan bo danh muc Laptop"""
    print("[*] Dang kiem tra tong so san pham Laptop tren CellphoneS...")
    
    all_products = []
    seen_skus = set()
    page = 1
    
    while True:
        print(f"    - Dang tai trang {page}...")
        prods = fetch_page(page)
        if not prods:
            break
            
        new_count = 0
        for p in prods:
            url_path = p.get('general', {}).get('url_path', '')
            if url_path not in seen_skus:
                seen_skus.add(url_path)
                all_products.append(p)
                new_count += 1
                
        if new_count == 0:
            break
            
        page += 1
        
    print(f"[*] Tim thay tong cong {len(all_products)} san pham ({page-1} trang).")
    return all_products

def get_brand_from_name(name):
    if not name:
        return ""
    words = name.split()
    if len(words) > 1 and words[0].lower() == "laptop":
        return words[1].upper()
    return words[0].upper()

def parse_item(item):
    general = item.get('general', {})
    filterable = item.get('filterable', {})
    
    name = general.get('name', 'N/A')
    url_path = general.get('url_path', '')
    link = f"https://cellphones.com.vn/{url_path}" if url_path else "N/A"
    
    price = filterable.get('special_price', 0)
    price_before = filterable.get('price', 0)
    if not price and price_before:
        price = price_before
        
    discount = "0%"
    if price_before > price and price_before > 0:
        discount = f"{round((1 - price/price_before) * 100)}%"

    attrs = general.get('attributes', {})
    
    cpu = ram = storage = vga = screen = ""
    if isinstance(attrs, dict) and attrs:
        cpu = attrs.get('cpu', '')
        ram = attrs.get('laptop_ram', '')
        storage = attrs.get('o_cung_laptop', attrs.get('hdd_sdd', ''))
        vga = attrs.get('vga', '')
        screen = attrs.get('display_size', '')

    brand = get_brand_from_name(name)
    
    specs_dict = {
        "Thương hiệu": brand,
        "CPU": cpu,
        "RAM": ram,
        "Ổ cứng": storage,
        "VGA": vga,
        "Màn hình": screen
    }
    specs_string = " | ".join([f"{k}: {v}" for k, v in specs_dict.items() if v])

    crawl_time = datetime.now(timezone(timedelta(hours=7))).strftime("%Y-%m-%d %H:%M:%S")
    
    try:
        current_price_str = f"{int(price):,}đ".replace(",", ".") if price > 0 else ""
        original_price_str = f"{int(price_before):,}đ".replace(",", ".") if price_before > 0 else ""
    except:
        current_price_str = ""
        original_price_str = ""

    return {
        "Tên Sản Phẩm": name,
        "Giá Hiện Tại": current_price_str,
        "Giá Gốc": original_price_str,
        "Giảm Giá": calculate_discount(current_price_str, original_price_str, discount),
        "Khuyến Mãi": "",
        "Quà Tặng / Ghi Chú": "",
        "Cấu Hình Chi Tiết": specs_string,
        "Link Sản Phẩm": link,
        "Ngày Cập Nhật (Sitemap)": "",
        "Ngày Giờ Crawl": crawl_time
    }

def crawl_cellphones_to_excel(chunk=1, total_chunks=1, get_links_only=False):
    timestamp = int(time.time())
    EXCEL_FILE = f"laptop_cellphones_chunk_{chunk}_{timestamp}.xlsx"
    
    all_products = get_all_laptop_products()
    
    if get_links_only:
        # Not needed since API call is fast, but we'll output empty links just in case
        LINKS_FILE = "cellphones_links.txt"
        with open(LINKS_FILE, "w", encoding="utf-8") as f:
            for p in all_products:
                url_path = p.get('general', {}).get('url_path', '')
                if url_path:
                    f.write(f"https://cellphones.com.vn/{url_path}\n")
        print(f"--> Đã lưu links ra file {LINKS_FILE}")
        return

    # Sort to ensure consistent order
    all_products = sorted(all_products, key=lambda x: x.get('general', {}).get('url_path', ''))
    
    chunk_size = math.ceil(len(all_products) / total_chunks)
    start_idx = (chunk - 1) * chunk_size
    end_idx = start_idx + chunk_size
    chunk_products = all_products[start_idx:end_idx]
    
    print(f"--> [SHARDING] Mảnh {chunk}/{total_chunks}: Cào {len(chunk_products)} sản phẩm (từ {start_idx} đến {end_idx-1})")

    parsed_products = [parse_item(p) for p in chunk_products]
    
    if parsed_products:
        df = pd.DataFrame(parsed_products)
        df.to_excel(EXCEL_FILE, index=False)
        print(f"\n[SAVED] {EXCEL_FILE}")
    else:
        print("\nKhong co du lieu nao de luu.")

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument('--chunk', type=int, default=1)
    parser.add_argument('--total-chunks', type=int, default=1)
    parser.add_argument('--get-links-only', action='store_true')
    args = parser.parse_args()
    
    print("=" * 60)
    print("  CELLPHONES - CRAWL ALL LAPTOP")
    print("=" * 60)
    
    crawl_cellphones_to_excel(chunk=args.chunk, total_chunks=args.total_chunks, get_links_only=args.get_links_only)
